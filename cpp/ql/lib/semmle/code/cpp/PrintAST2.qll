...
class PrintAstNode extends TPrintAstNode {
  ...
  string getProperty(string key) {
    key = "semmle.label" and result = toString()
    // ast-experiment changes (added key-value pairs)
    or
    key = "File" and result = getLocation().getFile().toString()
    or
    key = "Line_Start" and result = getLocation().getStartLine().toString()
    or
    key = "Line_End" and result = getLocation().getEndLine().toString()
    or
    key = "Column_Start" and result = getLocation().getStartColumn().toString()
    or
    key = "Column_End" and result = getLocation().getEndColumn().toString()
  }
...
abstract class BaseAstNode extends PrintAstNode {
  ...
  override string getProperty(string key) {
    ...
    or
    key = "Term" and result = qlClass(ast)
  }
}
...
class ExprNode extends AstNode {
  ...
  override string getProperty(string key) {
    result = super.getProperty(key)
    or
    // ast-experiment changes (changed/added key-value pairs)
    ...
    or
    key = "Element_Type" and result = qlClass(expr.getType())
  }
...
class CastNode extends ConversionNode {
  ...
  override string getProperty(string key) {
    result = super.getProperty(key)
    or
    // TODO we may need to split this to separate key-value pairs intead of string concatenation
    key = "Conversion" and
...
class DeclarationEntryNode extends BaseAstNode, TDeclarationEntryNode {
  ...
  override string getProperty(string key) {
    ...
    or
    // ast-experiment changes (changed/added key-value pairs)
    key = "Type" and result = ast.getType().toString()
    or
    key = "Element_Type" and result = qlClass(ast.getType())
    or
    key = "Name" and result = ast.getName()
  }
}
...
class InitializerNode extends AstNode {
  ...
  // ast-experiment changes (overrode getProperty)
  override string getProperty(string key) {
    result = super.getProperty(key)
    or
    key = "Name" and result = init.getDeclaration().getName()
  }
}
...
query predicate edges(PrintAstNode source, PrintAstNode target, string key, string value) {
  exists(int childIndex |
    ...
    (
      ...
      // TODO add additional edge key-value pairs here
    )
  )
}
